...
if [ "$(uname -o 2>/dev/null)" = "Android" ] || [ -n "$TERMUX_VERSION" ]; then
  # no Android, sharp/@roamhq/wrtc (deps do baileys 7) não têm binário
  # pré-compilado — ignoramos os scripts de build nativo (não são usados
  # pelo Lua). O better-sqlite3 é compilado por ensure_sqlite_binary.
