...
// ── validação do .env: nunca inicializar sem OWNER_NUMBER ──────────────
if (!CONFIG.owner.numbers.length) {
  console.error('ERRO: OWNER_NUMBER não configurado no .env');
...
const BAILEYS_VERSION = require('@innovatorssoft/baileys/package.json').version;
console.log(`Baileys: @innovatorssoft/baileys@${BAILEYS_VERSION}`);
...
logger.info({ tag: 'BOOT' }, `[LUA][BOOT] Baileys: @innovatorssoft/baileys@${BAILEYS_VERSION}`);
