...
async function applyFilters(sock, ctx) {
  ...
  if (f.antimedia || f.antiimagem || f.antivideo || f.antiaudio || f.antidocumento || f.antisticker || f.antiviewonce) {
  ...
  await tryDelete(sock, ctx);
  logger.info({ group: ctx.remoteJid, user: ctx.sender, action }, 'filtro acionado');
...
async function handleGroupParticipants(sock, ev) {
  ...
  for (const pid of participants) {
    if (action === 'add') {
      ...
      await maybeWelcome(sock, id, pid);
      ...
      await maybeGoodbye(sock, id, pid);
