...
const PROVIDERS = { local, api, fallback };

function providerOrder() {
  const mode = String(CONFIG.ai.provider || 'auto').toLowerCase();
  if (mode === 'local') return ['local', 'fallback'];
  if (mode === 'api') return ['api', 'fallback'];
  if (api.isConfigured()) return ['api', 'local', 'fallback'];
  ...
  for (const name of providerOrder()) {
    ...
    try {
      ...
      if (r && r.ok) {
        ...
        return { ...r, latencyMs, usedFallback: name !== 'api' };
      }
...
function status() {
  ...
  return {
    ...
    active: api.isConfigured() ? 'api (externa)' : 'local (offline)',
    order,
    model: api.isConfigured() ? CONFIG.ai.model : 'assistente-local',
    apiConfigured: api.isConfigured(),
    apiUrl: CONFIG.ai.apiUrl ? 'configurada' : '-',
