...
```
npm install @lucasmod/boruto_vk7-baileys
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:Otakump4/boruto_vk7-baileys
```

Then import your code using:
```ts 
import makeWASocket from '@lucasmod/boruto_vk7-baileys'
```
...
```ts
import makeWASocket, { useMultiFileAuthState } from '@lucasmod/boruto_vk7-baileys'
...
```ts
import makeWASocket, { DisconnectReason, useMultiFileAuthState } from '@lucasmod/boruto_vk7-baileys'
...
```ts
import makeWASocket, { useSingleFileAuthState, useMongoFileAuthState } from '@lucasmod/boruto_vk7-baileys'
...
const connectAuth = async() => {
  ...
  const collection = client.db("@boruto_vk7").collection("sessions")
  return collection
}
...
import { makeInMemoryStore, getAggregateVotesInPollMessage } from '@lucasmod/boruto_vk7-baileys'

const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: "@boruto_vk7" })
logger.level = "fatal"
...
```ts
import makeWASocket, { makeInMemoryStore } from '@lucasmod/boruto_vk7-baileys'
          ...
          { 
             ...
             buttonParamsJson: JSON.stringify({ 
                payment_settings: [{ 
                   ...
                   pix_static_code:  { 
                      merchant_name: 'boruto_vk7 kawaii >\\\\\\<', 
       ...
       interactiveButtons: [ 
          { 
             ...
             buttonParamsJson: JSON.stringify({ 
                ...
                additional_note: 'boruto_vk7 Kawaii >\\\<', 
...
import { downloadMediaMessage, getContentType } from '@lucasmod/boruto_vk7-baileys'
