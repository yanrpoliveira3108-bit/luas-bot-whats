...
const fetchLatestBaileysVersion = async (options = {}) => {
    // Fontes de versão em ordem de preferência. A primeira é o repositório
    // OFICIAL do Baileys (WhiskeySockets), atualizado diariamente; a segunda
    // é o repositório do fork (pode ficar desatualizado e causar erro 405
    // "client outdated" no WhatsApp). Se todas falharem, usa a versão embutida.
    const VERSION_URLS = [
    ...
    const timeoutMs = (options && options.timeout) || 15000
    let lastError = null
    for (const URL of VERSION_URLS) {
        try {
            const result = await axios_1.default.get(URL, {
                ...options,
                signal: undefined, // cada tentativa tem seu próprio timeout (axios timeout)
                responseType: 'json',
                timeout: timeoutMs
            })
            const v = result.data && result.data.version
            ...
            lastError = new Error('formato de versão inválido em ' + URL)
        }
        catch (error) {
            lastError = error
        }
