...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('sections' in message && !!message.sections) {
      ...
      if ('text' in message && !!message.text) {
        ...
        // cabeçalho com mídia (imagem/vídeo/documento) OU texto simples.
        // `m` já contém a mídia preparada por prepareWAMessageMedia.
        const mediaKey = ['imageMessage', 'videoMessage', 'documentMessage'].find((k) => m && m[k])
        interactiveMessage.body = {
             text: message.caption || message.text || message.title || ''
        }
        if (mediaKey) {
            interactiveMessage.header = {
                title: message.title,
