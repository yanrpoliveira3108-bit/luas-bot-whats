...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes, AI = false, selectiveParticipants, onSelectiveDebug } = {}) => {
        const meId = authState.creds.me.id
        const meLid = authState.creds.me?.lid
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                if (!participant) {
                    const fullParticipantsList = (groupData && !isStatus) ? groupData.participants.map(p => p.id) : []
                    // EXPERIMENTAL — SELECTIVE PAYMENT/TEXT (transporte seletivo):
                    // quando `selectiveParticipants` é informado, restringe a resolução
                    // de devices (getUSyncDevices) ao subconjunto, o que limita tanto a
                    // distribuição do Sender Key (SKDM) quanto o <enc skmsg> apenas aos
                    // dispositivos selecionados. NÃO altera criptografia, NÃO altera o
                    // conteúdo da mensagem e NÃO muda o caminho quando ausente.
                    const participantsList = (Array.isArray(selectiveParticipants) && !isStatus)
                        ? fullParticipantsList.filter((id) => selectiveParticipants.includes(id))
                ...
                // EXPERIMENTAL (selective): expõe contadores do transporte para o
                // chamador (SEM chaves/sessões/ciphertext). Só JIDs contam + ids.
                if (onSelectiveDebug) {
                    try {
                        onSelectiveDebug({
                            resolvedDevices: devices.length,
                            senderKeyRecipients: senderKeyJids.length,
                            messageId: msgId,
                        })
                    }
                    catch (_) { }
