...
#    sem --force cego.
#
#    O motor agora é o @lucasmod/boruto-vk7-baileys 2.1.0 (vendored em
#    vendor/boruto-vk7-baileys, com libsignal @itsukichan/libsignal-node).
#    Ele é 100% JavaScript (sem sharp/@roamhq/wrtc nativos), então no
#    Termux/Android usamos --ignore-scripts para não tentar compilar nada
#    desnecessário; o better-sqlite3 — que o Lua usa de verdade — é
#    compilado logo depois por ensure_sqlite_binary.
#
...
if [ "$(uname -o 2>/dev/null)" = "Android" ] || [ -n "$TERMUX_VERSION" ]; then
  ...
  echo "   (Termux: scripts de build nativo ignorados — motor é JS puro)"
fi
