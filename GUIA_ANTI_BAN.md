...
> ⚠️ **Atualização (unificação dos sistemas):** este guia foi escrito junto com
> a camada `utils/antiBan.js` (presença humana, silent PV, fingerprint de
> navegador). Ele convive com o **freio de envio** (`utils/sendGuard.js`), que é
> quem agora controla o ritmo (uma fila só, teto por minuto, pausa automática).
> Um ajuste importante: **cards HTML e menu por listas/botões ficaram LIBERADOS
> por padrão** — o dono testa vários bots que usam esses payloads sem cair, e o
> `!ping` (texto puro) também deu restrição. A hipótese "payload causa ban" não
> se sustentou. Para ver os fatos e a auditoria do seu caso:
> **`SEGURANCA-ENVIO.md`** e `node scripts/restricao.js`.
> Status/controle em tempo real: `!antiban` (status) e `!freio` (painel).
...
A Meta **descontinuou o suporte oficial a botões no WhatsApp Web**, e payloads
customizados (`interactiveButtons`, `buttonsMessage`) chamam atenção dos filtros.
**Ressalva importante:** não há prova de que isso, sozinho, cause restrição —
vários bots usam e não caem. Trate como fator de risco, não como causa
comprovada. No Lua, esses payloads estão **liberados por padrão** (`SAFE_MODE=0`)
e podem ser bloqueados com `!freio seguro on` se você quiser testar sem eles.
...
# ---- Menus (opcional) ----
# Botões/listas nativos são um fator de risco, não causa comprovada.
# Se quiser menu textual: BUTTONS_ENABLED=false + LUA_UI_MODE=text
# (o equivalente em runtime é `!freio seguro on`)
BUTTONS_ENABLED=true
LUA_UI_MODE=auto
```

> ℹ️ A fila de saída é única: quem controla ritmo/intervalo/teto é o
> `utils/sendGuard.js` (bloco `SEND_*` do `.env`). `OUTBOUND_INTERVAL_MS` só é
> usado se você definir `ANTI_BAN_OWN_QUEUE=1` (não recomendado — duplica a espera).
