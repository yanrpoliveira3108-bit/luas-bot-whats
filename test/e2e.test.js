 ...
 *
 * Simula: !vida (criação), !menu (lista nativa com imagem no cabeçalho),
 * !trabalho (tela de profissões), seleção de lista (single_select) → !emprego,
...
const DB = require('./dbtmp').tmpFile('lua-e2e-test.db');
...
async function main() {
  ...
  /* 2) !menu (botões ON) envia UMA lista nativa com imagem no cabeçalho */
  try {
    sent.length = 0;
    const ctx = await ctxFor(sock, mkMsg({ message: { conversation: '!menu' } }));
    await commandHandler.runByName(ctx, 'menu');
    const list = sent.find((s) => s.content && Array.isArray(s.content.interactiveButtons));
    assert.ok(list, 'enviou lista nativa (interactiveButtons)');
    assert.ok(Buffer.isBuffer(list.content.image), 'imagem do menu acompanha a lista (mesma mensagem)');
    assert.ok(/MENU PRINCIPAL/.test(list.content.caption || ''), 'título no corpo da lista');
    const params = JSON.parse(list.content.interactiveButtons[0].buttonParamsJson);
  ...
  try {
    ...
    const list = sent.find((s) => s.content && Array.isArray(s.content.interactiveButtons));
    assert.ok(list, 'tela de profissões com lista');
    const params = JSON.parse(list.content.interactiveButtons[0].buttonParamsJson);
    const ids = (params.sections || []).flatMap((sec) => sec.rows || []).map((r) => r.id);
    assert.ok(ids.some((i) => i.includes('job_faxineiro')), 'profissão presente na lista');
    ok('3: !trabalho abre profissões');
  } catch (e) { fail('3: !trabalho', e); }

  /* 4) seleção de lista (single_select) → !emprego executa */
  try {
    const click = mkMsg({
      message: {
    ...
    assert.strictEqual(handled, true, 'seleção processada');
    assert.strictEqual(rpg.getPlayer(OWNER).profession, 'Faxineiro', 'profissão definida pela seleção');
    ok('4: seleção de lista → !emprego');
  } catch (e) { fail('4: seleção lista', e); }
  ...
  try {
    ...
    const last = sent[sent.length - 1];
    // agora !minerar responde com imagem + legenda (assets/actions/minerar.jpg)
    const minerText = (last && (last.content.text || last.content.caption)) || '';
    assert.ok(/MINERAÇÃO/.test(minerText), 'resposta de mineração');
